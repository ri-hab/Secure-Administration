...
class ShellHandler(object):
    ...
    def __init__(self, ip, username, port, method, credential, timeout = 3, channel_name = None):
        if method not in ['key','password']:
            raise Exception('Authentication method must be key or password')
...
#start an interactive shell/terminal (invisible) session on the ssh server: get a dedicated channel/canal
#once you execute a cmd, a new channel is used, the execution finishes, the channel gets close and so on
#paramiko use channels to send cmds and receive output
#the channel will be used to interact with the invisible terminal: send keystokes strings to the terminal
...
class ShellHandlerThread(threading.Thread):
    ...
    def run(self):
        for server_ip in self.server_list:
            self.message.reply_channel.send({"text":json.dumps(['stdout','\033[1;3;31mExecute task on server: {} \033[0m'.format((smart_text(server_ip))) ] )},immediately=True)

            #get server credential info
            serverdata = Server.objects.get(ip=server_ip,credential__protocol__in=['ssh-password','ssh-key'])
            port = serverdata.credential.port
            method = serverdata.credential.method
            username = serverdata.credential.user.username
            if method == 'password':
